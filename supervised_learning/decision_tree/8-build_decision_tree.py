...
class Decision_Tree:
    ...
    def possible_thresholds(self, node, feature):
        """Return possible split thresholds for a feature."""
        values = np.unique(
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        ...
        n_individuals = target.size

        is_class = target[:, np.newaxis, np.newaxis] == classes
        is_left = (
            values[:, np.newaxis, np.newaxis]
            > thresholds[np.newaxis, :, np.newaxis]
        )
        is_right = np.logical_not(is_left)

        left_f = np.logical_and(is_class, is_left)
        right_f = np.logical_and(is_class, is_right)
        ...
        left_gini = 1 - np.sum((left_counts / left_total[:, None]) ** 2,
                               axis=1)
        right_gini = 1 - np.sum((right_counts / right_total[:, None]) ** 2,
                                axis=1)
        gini_split = (
            left_total * left_gini + right_total * right_gini
        ) / n_individuals

        index = np.argmin(gini_split)
        return thresholds[index], gini_split[index]
